...
from vi.app.nonogram import build_domain, reduce_domain, is_valid_pattern

def test_is_valid_pattern():
    assert not is_valid_pattern(2, [1], 0)
    assert is_valid_pattern(2, [1], 1)
    assert is_valid_pattern(2, [1], 2)
    assert not is_valid_pattern(2, [1], 3)
